...
def test(data_dir):
    ...
    # load model
    model = VolumeNet()
    model.load_state_dict(torch.load("models/volume_model_no_vol.pth"))
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            cv2.putText(
                img,
                "Predicted Liquid Volume: " + str(int(predicted_vol_liquid)),
                (10, 30),
            ...
            cv2.putText(
                img,
                "Real Liquid Volume: " + str(int(actual_vol_liquid)),
                (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX,
                1,
                (0, 0, 255),

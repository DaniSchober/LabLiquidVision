...
class VesselCaptureDataset(Dataset):
    def __init__(self, data_dir, transform=False):
        ...
        for sample_dir in os.listdir(data_dir):
            ...
            if os.path.isdir(sample_path):
                ...
                segmentation_vessel_path = os.path.join(sample_path, "Input_VesselMask.npy")
                segmentation_liquid_path = os.path.join(sample_path, "Input_ContentMaskClean.npy")
    ...
    def __getitem__(self, index):
        ...
        if self.transform == True:
            ...
            segmentation_liquid = F.interpolate(segmentation_liquid.unsqueeze(0), size=(160, 214), mode='bilinear', align_corners=False)
            segmentation_liquid = segmentation_liquid.squeeze(0).squeeze(0)
            segmentation_vessel = torch.from_numpy(segmentation_vessel)
            segmentation_vessel = F.interpolate(segmentation_vessel.unsqueeze(0), size=(160, 214), mode='bilinear', align_corners=False)

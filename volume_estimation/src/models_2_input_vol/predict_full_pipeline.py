...
def predict_with_vol(image_path, predict_volume=False, save_segmentation=False, save_depth=False, vessel_volume=0, no_GPU=False):
    ...
    if predict_volume == True:
        ...
        with torch.no_grad():
            model.eval()
            outputs = model(vessel_depth, liquid_depth, vessel_volume_torch)

        print(f"Predicted liquid volume: {round(outputs[0].item())} mL")
        ...
        plt.text(
            0.5,
            0.5,
            f"Predicted liquid volume: {round(outputs[0].item())} ml",
            ha="center",
            va="center",
            fontsize=12,
        )

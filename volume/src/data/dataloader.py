    ...
    def __init__(self, data_dir):
        ...
        for sample_dir in os.listdir(data_dir):
            ...
            if os.path.isdir(sample_path):
                ...
                if os.path.exists(color_image_path) and os.path.exists(depth_image_path) and os.path.exists(vol_liquid_path) and os.path.exists(vessel_path) and os.path.exists(vol_vessel_path):
                    self.samples.append((color_image_path, depth_image_path, vol_liquid_path, vessel_path, vol_vessel_path))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, index):
        color_image_path, depth_image_path, vol_liquid_path, vessel_path, vol_vessel_path = self.samples[index]
        ...
        return {'color_image': torch.from_numpy(color_image),

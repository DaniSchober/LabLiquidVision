...
void UpdateFrame()
{
	...
	if (g_capture)
	{
		...
		std::cout << img.m_width << " " << img.m_height << "\n";
		/*std::cout << "ffmpeg " << g_ffmpeg << "\n";*/
...
int main(int argc, char* argv[])
{
	...
	/*hFind = FindFirstFile("../../../../glass/sor_generator/output/top_and_bottom/*.*", &data);
	...
	// int count = 213; // End of sinusoids
	//g_capture = true;
	int count =  1792; // end of flasks
	std::string base_path = "../../../../glass/sor_generator/output/small/autogenerated_";
	std::string base_output_path = "../../output/wait_times_fix_random/autogenerated_";
	//g_scenes.push_back(new Pouring("Pouring Scene", "../../../../glass/sor_generator/output/spiral", "../../output/spiral"));
	/*for (float v =0; v<1; v+=0.1)
		for (float c=0.; c < 0.1; c+= 0.01)
			g_scenes.push_back(new Pouring("Pouring Scene", base_path + "1", base_output_path + "1" + "c" + std::to_string(c) +"v"+std::to_string(v), v, c));*/
	/*for (int i = 0; i < count; i++) {
		g_scenes.push_back(new Pouring("Pouring Scene", base_path + std::to_string(i), base_output_path + std::to_string(i) + "_0.2", 0.0, 0.0, 0.2));
		g_scenes.push_back(new Pouring("Pouring Scene", base_path + std::to_string(i), base_output_path + std::to_string(i) + "_0.1", 0.0, 0.0, 0.1));
		g_scenes.push_back(new Pouring("Pouring Scene", base_path + std::to_string(i), base_output_path + std::to_string(i) + "_0.05", 0.0, 0.0, 0.05));
		g_scenes.push_back(new Pouring("Pouring Scene", base_path + std::to_string(i), base_output_path + std::to_string(i) + "_0.025", 0.0, 0.0, 0.025));
	}*/
	...
	for (int j = 5; j < 15; j++) {
		...
		if (j ==1) {
			for (int i = count; i >0; i -= 1) {
				if (i > 142 && i < 213) continue;
				if (i  < 1230) continue;
				
				g_scenes.push_back(new Pouring("Pouring Scene", base_path + std::to_string(i), base_output_path + std::to_string(i) + "iteration_" + std::to_string(j), ));
			}
			...
			for (int i = 0; i < count; i += 1) {
				if (i > 142 && i < 213) continue;
				if (i  < 614) continue;
				g_scenes.push_back(new Pouring("Pouring Scene", base_path + std::to_string(i), base_output_path + std::to_string(i) + "iteration_" + std::to_string(j), 0.0, 0.0));
			}
	...
	*/
	...
	g_scenes.push_back(new Pouring("Pouring Flask", "../../data/Assembly_Flask_TCP_y58_x18575", "../../output/CellFlask"));
	g_scenes.push_back(new Pouring_Bottle("Pouring Bottle", "../../data/Assembly_MediumBottle_TCP_65y_50x", "../../output/MediumBottle"));
	g_scenes.push_back(new Calibration("Calibration Scene", "../../data/Cylinder_500ml"));

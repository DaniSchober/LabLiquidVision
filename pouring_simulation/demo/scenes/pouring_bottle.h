...
// This scene creates a pouring simulation with a bottle. The bottle rotates around the CoR, stops at a certain angle, and then returns to its initial position.
...
public:
	...
	virtual void Initialize()
	{
		...
		rotationSpeed = 0.03; // constant rotation speed in rad/s
	...
	bool InPouringContainer(Vec4 position, float theta) {
		return position.y > TCP_y - 3; // returns true if the particle is in the pouring container
	}

	bool InReceivingFlask(Vec4 position){
		return position.y > 0.22188 && position.y < 4; // returns true if the particle is in the receiving container
	}
	...
	void Update()
	{
		...
		if (continous) {
			...
			if (!stopped) { // if the container is not stopped, rotate it
		...
		else if (return_activated && prev_theta > 0) { // if container has reached the stop angle, start the return
			if (!pause_complete) {
				if (time - pause_start > pause_time) { // if the pause time is over, start returning
					pause_complete = true;
					theta = prev_theta;
					prev_pos_x = pos_x;
					prev_pos_y = pos_y;
				}
				else { 								// if the pause time is not over, keep the container in the same position
					theta = prev_theta;
					pos_x = prev_pos_x;
					pos_y = prev_pos_y;
				}
			}
			else { // if the pause time is over, continue returning
				theta = prev_theta;
				prev_pos_x = pos_x;
				prev_pos_y = pos_y;
		...
		// write results for each step in text files
		if (mTime > startTime) {
